...
func (api *API) PostTrips(w http.ResponseWriter, r *http.Request) *spec.Response {
	...
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		return spec.PostTripsJSON400Response(spec.Error{Message: "Cannot parse body: " + err.Error()})
	}
	...
	go func() {
		if err := api.mailer.SendConfirmTripEmailToTripOwner(tripID); err != nil {
			api.logger.Error("failed to send confirm trip email", zap.Error(err), zap.String("trip_id", tripID.String()))
		}
	}()
