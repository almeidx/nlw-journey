...
func (q *Queries) CreateTrip(ctx context.Context, pool *pgxpool.Pool, params spec.CreateTripRequest) (uuid.UUID, error) {
	...
	if err != nil {
		return uuid.Nil, fmt.Errorf("pgstore: failed to begin transaction for CreateTrip: %w", err)
	}
	...
	if _, err := qtx.InviteParticipantsToTrip(ctx, participants); err != nil {
		return uuid.Nil, fmt.Errorf("pgstore: failed to invite participants for CreateTrip: %w", err)
	}

	if err := tx.Commit(ctx); err != nil {
		return uuid.Nil, fmt.Errorf("pgstore: failed to commit transaction for CreateTrip: %w", err)
	}
